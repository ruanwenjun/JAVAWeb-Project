...
<script type="text/javascript">
	...
	$(function() {
		...
		// 基本功能菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/userAction_menuAjax',
			type : 'POST',
			dataType : 'text',
		...
		$("#btnEp").click(function(){
			//进行表单校验
			if($("#passwordEditForm").form('validate')){
				//判断两次密码是否相同
				var pw1 = $("#txtNewPass").val();
				var pw2 = $("#txtRePass").val();
				//判断两次密码输入是否一致
				if(pw1 == pw2){
					//两次密码输入一致
					$.messager.confirm('消息提示', '确认修改密码吗?', function(r){
						//点击确认修改密码
						if (r){
							$.post("${pageContext.request.contextPath}/userAction_editPassword",{password:pw1},
									function(data){
										if(data=="1"){
											//修改成功，关闭修改密码窗口
											$('#editPwdWindow').window('close');
					...
					//两次密码输入不一致，弹出提示
					$.messager.alert('消息提示','两次密码不一致!','error');  
				}
			}else {
				//表单校验不成功，弹出提示
				$.messager.alert('消息提示','密码输入不符合要求!','error');  
			}
		});
	});
